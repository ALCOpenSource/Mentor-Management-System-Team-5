...
namespace mms.Application.MentorManagers.Query
{
            ...
            IResult<List<GetMentorManagersResponse>>>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var mentors = await _context.MentorManagers.Include(M => M.ProgramsMentors).Include(y => y.ProgramMentorManagers).ThenInclude(r => r.Program).Include(x => x.AppUser).Where(x => x.AppUserId == _currentUserService.AppUserId).ToListAsync();
            if (!mentors.Any())
            {
